...
var TerraformPluginSDKExternalNameConfigs = map[string]config.ExternalName{
	...
	"azurerm_private_dns_resolver_inbound_endpoint":     config.TemplatedStringAsIdentifier("name", "/subscriptions/{{ .setup.configuration.subscription_id }}/resourceGroups/{{ .parameters.resource_group_name }}/providers/Microsoft.Network/dnsResolvers/{{ .parameters.azurerm_private_dns_resolver }}/inboundEndpoints/{{ .external_name }}"),
