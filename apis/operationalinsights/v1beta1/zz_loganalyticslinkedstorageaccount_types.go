...
type LogAnalyticsLinkedStorageAccount struct {
	...
	// +kubebuilder:validation:XValidation:rule="!('*' in self.managementPolicies || 'Create' in self.managementPolicies || 'Update' in self.managementPolicies) || has(self.forProvider.dataSourceType)",message="dataSourceType is a required parameter"
