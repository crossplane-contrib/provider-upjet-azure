...
func main() { //nolint:gocyclo // easier to follow as a unit
	...
	if startWebhooks {
		kingpin.FatalIfError(clustercontroller.SetupWebhookWithManager_botservice(mgr), "Cannot setup cluster-scoped webhooks")
		kingpin.FatalIfError(namespacedcontroller.SetupWebhookWithManager_botservice(mgr), "Cannot setup namespaced webhooks")
	}
	...
	if canSafeStart {
		...
		kingpin.FatalIfError(clustercontroller.SetupGated_botservice(mgr, clusterOpts), "Cannot setup cluster-scoped Azure controllers")
		kingpin.FatalIfError(namespacedcontroller.SetupGated_botservice(mgr, namespacedOpts), "Cannot setup namespaced Azure controllers")
	} else {
		logr.Info("Provider has missing RBAC permissions for watching CRDs, controller SafeStart capability will be disabled")
		kingpin.FatalIfError(clustercontroller.Setup_botservice(mgr, clusterOpts), "Cannot setup cluster-scoped Azure controllers")
		kingpin.FatalIfError(namespacedcontroller.Setup_botservice(mgr, namespacedOpts), "Cannot setup namespaced Azure controllers")
	}
