...
func main() {
	...
	if canSafeStart {
		...
		kingpin.FatalIfError(clustercontroller.SetupGated_mixedreality(mgr, clusterOpts), "Cannot setup cluster-scoped Azure controllers")
		kingpin.FatalIfError(namespacedcontroller.SetupGated_mixedreality(mgr, namespacedOpts), "Cannot setup namespaced Azure controllers")
	} else {
		logr.Info("Provider has missing RBAC permissions for watching CRDs, controller SafeStart capability will be disabled")
		kingpin.FatalIfError(clustercontroller.Setup_mixedreality(mgr, clusterOpts), "Cannot setup cluster-scoped AzureAD controllers")
		kingpin.FatalIfError(namespacedcontroller.Setup_mixedreality(mgr, namespacedOpts), "Cannot setup namespaced AzureAD controllers")
	}
